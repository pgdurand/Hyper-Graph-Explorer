...
public class HGEEngine {
   ...
   *          a helper search map
   */
  public HGEEngine(DataGraphModel model, HDataGraph graph, HGEQueryDeclaration query, List<String> searchMap) {
    this();
    _query = query;
    _dGraph = graph;
    _model = model;
   ...
   * 
   * @param b
   *          pass true to enable verbose mode.
   */
  ...
  private void createVarToIdxMap() {
    ...
    while (enum1.hasMoreElements()) {
      ...
      map.put(var.getName(), new Integer(idx));
      idx++;
    }
  ...
  /*
   * Optimisation possible: cette fonction est appelee par scanGraph() lorsqu'on
   * avance d'un pas dans la recherche. Plutot que d'allouer plein de tableaux,
   * il faudrait allouer un tableau de n entrees (n=nb. de pas de recherche).
   * Chacune de ces entrees est un tableu de taille m (m= nb. de variables de la
   * requetes).
   */
   ...
   * data is stored as a key/value pairs set. When a key is a vertex/edge
   * variable, value is a graph object (DGVertex/DGEdge). When key is a path
   * variable, value is a List (of DGEdge). When key is a graph variable, value
   * is a HGEResult object itself.
   */
  ...
  private void scanEdge(Set<HGEResult> rSet, Object[] result, int from, int idx, int to, HDGVertex vertex,
      HDGHyperEdge edge) {
    ...
    curVar = (HGENodeVarDeclaration) _query.getVariable(_searchMap.get(idx).toString());
  ...
  private void scanGraph(Set<HGEResult> rSet, Object[] result, int from) {
    ...
    if (from >= _searchMapSize) {
      ...
      if (var instanceof HGEEdgeVarDeclaration) {
        ...
        nVar = (HGENodeVarDeclaration) _query.getVariable(_searchMap.get(from).toString());
        ...
        if (_verbose) {
          ...
          System.out.println(spacer(from) + "search: " + eVar.getName() + " (" + result[_varToIdxMapper[from + 1]] + ")"
              + " from: " + nVar.getName() + " (" + vertex + ")" + " to: " + szBuf.toString());
        }
        // stop - print help
        ...
        while (enum1.hasMoreElements()) {
          ...
          if (evaluateTypeConstraint(edge, eVar.getType())) {
            ...
            scanEdge(rSet, copyResult(result), from, from + 2, from + 2 + nNodeVars, vertex, edge);
          }
        }
        ...
        vertex = (HDGVertex) result[_varToIdxMapper[from]];
        nVar = (HGENodeVarDeclaration) _query.getVariable(_searchMap.get(from + 2).toString());
  ...
  public boolean execute() {
    ...
    try {
      ...
      nVar = (HGENodeVarDeclaration) _query.getVariable(_searchMap.get(0).toString());
      _nResult = 0;
      while (enum1.hasMoreElements()) {
